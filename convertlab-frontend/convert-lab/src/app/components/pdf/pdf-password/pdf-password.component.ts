...
import { Action } from '../../../models/pdf-password.model';
...
@Component({
  ...
  styleUrl: './pdf-password.component.scss',
})
export class PdfPasswordComponent {
  ...
  actionType = signal<Action>(Action.ADD);
  ...
  passwordButtonLabel = computed(() => {
    if (this.isProcessing()) {
      if (this.actionType() === Action.ADD) {
        return 'Adding password...';
      } else {
  ...
  ngOnInit() {
    this.seoService.applySEO('pdf-password');
  }
